...
const apiKey = '4a4f8ba8aaa144ff98854ca97003b2e2';

app.get('/api/games/:page', async (req, res, next) => {
  try {
    const pageNumber = +req.params.page;
    const response = await fetch(
      `${targetUrl}?key=${apiKey}&page_size=20&page=${pageNumber}`
    );
    if (!response.ok) {
...
app.get('/api/details/:id', async (req, res, next) => {
  try {
    ...
    const response = await fetch(`${targetUrl}/${gameId}?key=${apiKey}`);
    if (!response.ok) {
      next(new Error(response.status.toString()));
    } else {
      const result = await response.json();
...
app.get('/api/search', async (req, res, next) => {
  try {
    ...
    const response = await fetch(`${targetUrl}?key=${apiKey}&search=${query}`);
